...
public class HttpClientUtils {
	...
	static {
		...
		client = HttpClients.custom().setDefaultRequestConfig(defaultConfig).setConnectionManager(cm).build();
	}
...
//              connection.setRequestProperty("apikey", "8ace4f47915ebed5baf86eef11827bda");
	...
	public static String get(String url) throws Exception {
		return get(url, URL_PARAM_DECODECHARSET_UTF8, connTimeout, readTimeout);
	}
	 ...
	 * @throws Exception
	 */
	public static String post(String url, byte [] content, String mimeType,
			String charset, Integer connTimeout, Integer readTimeout)
			throws ConnectTimeoutException, SocketTimeoutException, Exception {
		HttpClient client = null;
		...
		try {
			if (content != null && content.length > 0) {
				HttpEntity entity = new ByteArrayEntity(content, ContentType.create(
						mimeType, charset));
				post.setEntity(entity);
			}
			// 设置参数
	...
	private static CloseableHttpClient createSSLInsecureClient()
		...
		try {
			...
			return HttpClients.custom().setDefaultRequestConfig(defaultConfig).setSSLSocketFactory(sslsf).build();

		} catch (Throwable e) {
			logger.error("http创建SSL连接异常!", e);
			throw e;
		}
	}
